...
/// license synchronously (override snapshot, environment JWT, or the fallback tier); an invalid
/// configured JWT degrades to fallback-tier entitlements with <see cref="LicenseStatus.Invalid"/>
/// instead of failing the host. A license stored by the product is applied later via
...
internal sealed class LicenseService : ILicenseService
{
    ...
    public bool HasFeature(string feature) => current.Features.Contains(feature);

    public long GetLimit(string limit)
    ...
    private static bool FeaturesEqual(IReadOnlySet<string>? a, IReadOnlySet<string>? b)
    {
