...
/// <summary>
/// Generic repository for a domain entity backed by a stored EF entity: identity lookups, paging,
/// add/update/upsert with optimistic concurrency, an optional in-memory cache, and deferred change
/// notifications. Product repositories derive from this and supply an <see cref="IMapper{TDomain,TStored}"/>.
/// The context is threaded as a <see cref="DbContext"/> base, so the foundation is provider- and
/// product-context-agnostic.
/// </summary>
public abstract class AbstractRepository<TDomainEntity, TStoredEntity> : IRepository<TDomainEntity>
    ...
    where TStoredEntity : Entity
{
    ...
    /// afterwards, so that stale entity was served until the TTL expired. It carries a stale
    ...
    /// race into minutes of failing writes on hot, cached, mutated entities. Repeating the
    /// invalidation after the commit closes the window.
    /// </remarks>
    ...
    /// </summary>
    private ValueTask<TStoredEntity?> FindAsync(DbContext context, Guid id, CancellationToken cancellationToken = default)
    ...
    public virtual async Task<IReadOnlyList<TDomainEntity>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        // whole table and filtering (and caching the unfiltered set) afterwards.
        ...
        CancellationToken cancellationToken)
    {
        ...
        // UpdatedAt is configured as an EF concurrency token (see NordsteinDbContext.OnModelCreating),
    ...
    /// <remarks>
    /// <para>
    /// The relational path issues a single server-side <c>DELETE</c> rather than materializing every
    /// row into the change tracker just to delete it — which matters on the highest-volume tables in
    /// an install.
    /// </para>
    /// <para>
    /// A non-relational provider (e.g. the EF in-memory provider used by kiosk/tests) cannot translate
    /// <c>ExecuteDelete</c>, so it keeps the materializing path — cheap on those datasets.
    /// </para>
    /// <para>
    ...
    /// <summary>
    /// Realigns the <c>UpdatedAt</c> concurrency-token original value to the precision a relational
    /// store persists (microseconds) before a tracked update/archive is saved. A row inserted or
    /// updated earlier in this same context is still tracked at .NET's 100-nanosecond precision, which
    /// would otherwise spuriously fail the DB-level <c>WHERE UpdatedAt = @original</c> check against the
